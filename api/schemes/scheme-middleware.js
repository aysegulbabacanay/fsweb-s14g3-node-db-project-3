...
try{
const presentScheme= await db("schemes").where("scheme_id",req.params.scheme_id).first(); // scheme id olarak tanımlanmış routerda
if(!presentScheme){
  res.status(404).json({
  ...
  req.scheme=presentScheme // olmasa da çalışıyor
  next()
}
}
catch (err){
