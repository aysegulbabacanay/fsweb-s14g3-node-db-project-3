...
function find() {
  ...
  */
  return db("schemes as sc")
    ...
    .select("sc.scheme_id", "sc.scheme_name") // elde etmek istediğim data fieldlernı yazarız.
    .count("st.step_number as number_of_steps") // st.step_number as ya da st.step_id yazılabilir. toplam steps sayısını gösteriyor. // select ile sayısını sayamadığı için count kullanılıyor.
    .groupBy("sc.scheme_id") //bu olmassa 1 adet getiriyor. sc.scheme_name yazsak da oluyor.
    .orderBy("sc.scheme_id", "asc");//sıralamak
}
...
async function findById(scheme_id) {
  ...
  */
  ...
  const result = {
    scheme_id: records[0].scheme_id, //dizinin herhangi bir elemanı da olurdu .(records [2]) çünkü scheme_name ve scheme_id hepsinde aynı
  ...
  records.forEach((row) => {
    if (row.step_id) {
      result.steps.push({
        step_id: row.step_id, //row.step_id yi step_id ye tanımlıyorum.
...
async function findSteps(scheme_id) {
  ...
  */
  const records = await db("schemes as sc")
    ...
    .where("sc.scheme_id", scheme_id)
    .orderBy("st.step_number", "asc")
  if (!records[0].step_id) {  // 7. id nin step i yok .bu yüzden 0 aldık
    return []
  } 
  else {
    return records
